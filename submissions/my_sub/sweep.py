...
def _name_strategy(ema_kw: Dict, est_kw: Dict, sel_kw: Dict) -> str:
    parts: List[str] = []
    ...
    for ema_combo in itertools.product(*ema_vals):
        for est_combo in itertools.product(*est_vals):
            for sel_combo in itertools.product(*sel_vals):
                ...
                name = _name_strategy(ema_kw, est_kw, sel_kw)
                strat = submission.Strategy(
                    ema=ema_cfg,
                    estimate_w=est,
                    build_deployment=bld_fn,
                    select_layers=sel,
                    name=name,
                )
...
def main() -> None:
    ...
    # Sweep with live CSV append + periodic best-so-far summary.
    ...
    with out_path.open("w", newline="", buffering=1) as f:
        ...
        for model, ep in cases:
            ...
            for name, strat in strat_list:
                n += 1
                print(f"[{n:>3}/{total}] {model} EP{ep:<3}  {name}", end="", flush=True)
                ...
                # Periodic best-so-far snapshot.
                if n % 20 == 0 and n < total:
                    best = sorted(rows, key=lambda r: -r["score"])[:top]
                    print(f"  --- best so far ({n}/{total}) ---")
                    for r in best:
    ...
    print(f"\nFinal top {min(top, len(rows))} rows by score:")
